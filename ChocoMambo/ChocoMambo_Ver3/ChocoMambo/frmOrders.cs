...
namespace ChocoMambo
{
    public partial class frmOrders : Form
    {
        ...
        private void populateProductPriceCombo()
        {
            cboPrice.DataSource = _order.getProduct();
        ...
        private void assignChildData()
        {
            ...
            _order.OrderLineClass.ProductPrice = decimal.Parse(cboPrice.SelectedValue.ToString().Substring
                                                          (cboPrice.SelectedValue.ToString().IndexOf('$') + 1));
            _order.OrderLineClass.OrderNumber = _order.PKID;
            _order.OrderLineClass.OrderLineSubTotal = decimal.Parse(txtLineTotal.Text.Substring
                                                          (txtLineTotal.Text.IndexOf('$') + 1));
        ...
        private void mnuInsert_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                calculateGrandTotal();
                emptyOrderLineControls();
                if (cboCustomer.SelectedValue.ToString() != "" || dtpOrderDate.Value != null)
                {
